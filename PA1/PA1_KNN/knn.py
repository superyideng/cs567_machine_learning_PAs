...
class KNN:
    ...
    def predict(self, features):
        ...
        for feature in features:
            k_neighbors_label = self.get_k_neighbors(feature)
            current_label = Counter(k_neighbors_label).most_common(1)[0][0]
    ...
    def get_k_neighbors(self, point):
        ...
        for i in range(len(self.features)):
            distances.append(self.distance_function(point, self.features[i]))
        ...
        for idx in k_neighbors_idx:
            k_neighbors_label.append(self.labels[idx])
