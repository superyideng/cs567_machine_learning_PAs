...
class linear_layer:
    ...
    def __init__(self, input_D, output_D):
        ...
        shape_W = (input_D, output_D)
        shape_b = (1, output_D)

        self.params = dict()

        ###############################################################################################
        # Use np.random.normal() with mean as 0 and standard deviation as 0.1
        # W Shape (input_D, output_D), b shape (1, output_D)
        ###############################################################################################
        self.params['W'] = np.random.normal(0, 0.1, shape_W)
        self.params['b'] = np.random.normal(0, 0.1, shape_b)
        ...
        ###############################################################################################
        self.gradient['W'] = np.zeros(shape_W)
        self.gradient['b'] = np.zeros(shape_b)
    ...
    def forward(self, X):
        ...
        ################################################################################
        # Implement the linear forward pass. Store the result in forward_output  #
        ################################################################################
    ...
    def backward(self, X, grad):
        ...
        #################################################################################################
        # Implement the backward pass (i.e., compute the following three terms)
        ...
        backward_output = grad.dot(np.transpose(self.params['W']))
...
class relu:
    ...
    def forward(self, X):
        ...
        ################################################################################
        zero_mat = np.zeros((np.size(X, 0), np.size(X, 1)))
        forward_output = np.maximum(X, zero_mat)
...
class tanh:
    ...
    def forward(self, X):
        ...
        ################################################################################
        # Implement the tanh forward pass. Store the result in forward_output
        # You can use np.tanh()
        ################################################################################
    ...
    def backward(self, X, grad):
        ...
        ####################################################################################################
        tanh_X = np.tanh(X)
        backward_output = np.multiply(grad, (1 - np.power(tanh_X, 2)))
        return backward_output
        ...
        if hasattr(module, 'params'):
            for key, _ in module.params.items():
                ...
                if _alpha > 0.0:
                    ...
                    #################################################################################
                    # Update momentun using the formula:
                    ...
                    #################################################################################
                    # Update model parameter without momentum
                    #################################################################################
